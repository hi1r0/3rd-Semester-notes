...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        card/.style={
            ...
            font=\sffamily, align=left, inner sep=7pt, text width=5.8cm,
            minimum height=3.6cm
        }
    ]
        % Main Timeline Axis
        \draw[timeline] (0, 0) -- (14.8, 0) node[right, font=\sffamily\bfseries\small] {Time};

        % Milestone 1: Trade Date T
        \filldraw[navyblue] (1.0, 0) circle (3pt);
        \node[badge, draw=navyblue, fill=softblue] at (1.0, 1.1) {Trade Date ($T$)\\[0.15em] \textmd{\tiny Contract Booked}};
        \draw[timepin] (1.0, 0.45) -- (1.0, 0);

        % Milestone 2: Spot Date T+2
        \filldraw[darkteal] (3.8, 0) circle (3pt);
        \node[badge, draw=darkteal, fill=softgreen] at (3.8, 1.1) {Spot Value Date ($T+2$)\\[0.15em] \textmd{\tiny FEDAI Rule 7 Tenor Base}};
        \draw[timepin, draw=darkteal] (3.8, 0.45) -- (3.8, 0);

        % 2-Day Spot Interval
        \draw[<->, draw=charcoal, line width=0.9pt] (1.0, -0.5) -- node[below, font=\tiny\sffamily\bfseries] {2 Working Days} (3.8, -0.5);

        % Milestone 3: Fixed Forward Maturity
        \filldraw[amber!90!black] (8.8, 0) circle (3pt);
        \node[badge, draw=amber!90!black, fill=softamber] at (8.8, 1.1) {Fixed Maturity ($T+2+n$)\\[0.15em] \textmd{\tiny Single Settlement Date}};
        \draw[timepin, draw=amber!90!black] (8.8, 0.45) -- (8.8, 0);

        % Tenor Duration Bracket
        \draw[<->, draw=darkteal, line width=1.1pt] (3.8, -1.2) -- node[below, font=\scriptsize\sffamily\bfseries] {Forward Tenor Computed from Spot Date (FEDAI Rule 7)} (8.8, -1.2);

        % Milestone 4: Option Delivery Window
        \fill[softblue, opacity=0.9] (10.8, -0.3) rectangle (14.2, 0.3);
        \draw[draw=navyblue, dashed, line width=1.1pt] (10.8, -0.3) rectangle (14.2, 0.3);
        \node[badge, draw=navyblue, fill=white] at (12.5, 1.1) {Option Window\\[0.15em] \textmd{\tiny Delivery on Any Day}};
        \draw[timepin] (12.5, 0.45) -- (12.5, 0.1);
        \draw[<->, draw=navyblue, line width=0.9pt] (10.8, -0.5) -- node[below, font=\tiny\sffamily\bfseries] {Max 1 Month (FEDAI)} (14.2, -0.5);

        % Symmetrical Cards Below
        \node[card, draw=amber!90!black, fill=softamber] at (3.5, -3.8) {
        ...
        \node[card, draw=navyblue, fill=softblue] at (10.5, -3.8) {
