...
## 5. Combined Multi-Chapter Formula Reference Section at Document End
- **Single Combined Section at Document End**: Do NOT generate separate formula reference sections at the end of each individual chapter. Instead, maintain a single, consolidated **Comprehensive Formula Reference Sheet** placed at the very end of the entire document (`main.tex`).
- **Chapter-by-Chapter Grouping**: Within the combined formula reference sheet, clearly demarcate formulas under distinct chapter headings (e.g. `\section*{Chapter 1: ...}`, `\section*{Chapter 2: ...}`).
- **Mandatory "Where:" Parameter Definitions**: Every formula box MUST explicitly provide a structured itemized list of parameter definitions introduced by **Where:** (e.g. `$P = \text{Theoretical market price per share}$`, `$k = \text{cost of capital}$`, `$r = \text{internal rate of return}$`, etc.).
- **Formula-Only Presentation**: This section must strictly present mathematical formulas, equations, variable definitions, and validity/payout conditions, with NO narrative commentary, prose introductions, or worked examples.
