...
\begin{align}
    \text{Annual Compounding:} \quad A &= P \left(1 + \frac{R}{100}\right)^N, \quad \CI = A - P = P \left[\left(1 + \frac{R}{100}\right)^N - 1\right] \\[4pt]
...
\begin{itemize}[noitemsep, topsep=2pt]
    ...
    \item $m$ = Number of compounding periods per year ($m=2$ semi-annual, $m=4$ quarterly, $m=12$ monthly)
    \item $e$ = Mathematical constant / base of natural logarithm ($\approx 2.71828$)
    \item $A$ = Compound terminal amount; $\CI$ = Compound interest earned
\end{itemize}
\end{formulabox}

\begin{formulabox}[3. Effective Annual Rate of Interest (ERI / EAR) and Depreciation]
\begin{align}
    \ERI &= \left(1 + \frac{R}{m \cdot 100}\right)^m - 1 \\[4pt]
    \text{Depreciation (Reducing Balance Method):} \quad A &= P \left(1 - \frac{R}{100}\right)^N
\end{align}
\textbf{Where:}
\begin{itemize}[noitemsep, topsep=2pt]
    \item $\ERI$ = Effective rate of interest (true annualized yield in decimal)
    \item $m$ = Compounding frequency per annum
    \item $P$ = Original asset acquisition cost
    \item $R$ = Annual rate of depreciation ($\%$)
    \item $N$ = Asset operational life in years
    \item $A$ = Written Down Value (WDV) or estimated salvage/scrap value
\end{itemize}
\end{formulabox}

\begin{formulabox}[{4. Doubling Period Approximations}]
\begin{align}
    \text{Exact Limit:} \quad T_{\text{exact}} &= \frac{\ln 2}{r} \approx \frac{0.6931}{r} = \frac{69.3}{R} \\[4pt]
    \text{Empirical Rules:} \quad T_{\text{Rule of 72}} &\approx \frac{72}{R}, \qquad T_{\text{Rule of 69}} \approx 0.35 + \frac{69}{R}
\end{align}
\textbf{Where:}
\begin{itemize}[noitemsep, topsep=2pt]
...
\begin{itemize}[noitemsep, topsep=2pt]
    ...
    \item $\CVAF$ = Compound Value Annuity Factor; $\PVF$ = Present Value Factor
    \item $\PVAF$ = Present Value Annuity Factor
...
\begin{equation}
    \text{Growth Model:} \quad n(t) = n_0 \, e^{k t}, \qquad \text{Decay Model:} \quad n(t) = n_0 \, e^{-k t}, \qquad k = \frac{1}{T} \ln\left(\frac{n_{\max}}{n_{\min}}\right)
\end{equation}
\textbf{Where:}
\begin{itemize}[noitemsep, topsep=2pt]
...
\begin{equation}
    P_0 = \frac{\EPS_1 (1 - b)}{K_e - b \cdot r} = \frac{D_1}{K_e - g}, \qquad g = b \times r
\end{equation}
\textbf{Where:}
\begin{itemize}[noitemsep, topsep=2pt]
...
\begin{equation}
    P = \frac{D + \frac{r}{k}(E - D)}{k}, \qquad V = N \times P
\end{equation}
\textbf{Where:}
\begin{itemize}[noitemsep, topsep=2pt]
